...
/**
 * Heuristic + LLM check to decide if a query is about the movie dataset domain.
 * Returns { isMovieDomain: boolean, reason: string }
 */
export async function classifyMovieDomain(query) {
  // quick heuristic first (cheap & fast)
  const movieish = /\b(movie|film|actor|actress|director|genre|imdb|box office|cinema|screenplay|cast|soundtrack|scene|sequel|prequel)\b/i
  const hasMovieHint = movieish.test(query)
  if (hasMovieHint) {
    return { isMovieDomain: true, reason: "Heuristic matched common movie terms." }
  }

  // if heuristic didn't match, ask the model (kept lightweight)
  try {
    const completion = await openai.chat.completions.create({
      model: "gpt-3.5-turbo",
      messages: [
        {
          role: "system",
          content:
            `Classify if the user's question is about MOVIES / film domain (titles, people in films, genres, box office, etc).
Return a compact JSON object { "isMovieDomain": true|false, "reason": "<short>" } and NOTHING else.`
        },
    ...
    const isMovieDomain = !!parsed.isMovieDomain
    const reason = typeof parsed.reason === "string" ? parsed.reason : "Classifier response."
    return { isMovieDomain, reason }
  } catch (e) {
    console.warn("Movie-domain classification fallback (error calling API):", e)
    // fallback: assume NOT movie domain if heuristic failed
    return { isMovieDomain: false, reason: "Fallback: heuristic did not match and API check failed." }
  }
}
