...
//     let graphData = buildGraph(movies);

//     // Step 5: Generate LLM responses
//     const context = formatContext(movies, queryAnalysis);

//     // If no movies found, set empty context
//     const ragAnswer = await generateResponse([
//       { 
//         role: "system", 
//         content: `You are a movie expert. Use ONLY the provided context to answer.
// Your answers can include markdown formatting for better readability, including bold text, lists, and headings.
// If the question is outside the movie dataset, or if the requested information is not in the provided context, respond politely with:
// "Sorry, the information you requested is not available in the movie dataset."
...
//     if (typeof ragAnswer === "string" && ragAnswer.toLowerCase().includes("not available in the movie dataset")) {
...
//       { 
//         role: "system", 
//         content: `You are a movie expert. No external context available.
// Your answers can include markdown formatting for better readability, including bold text, lists, and headings.
// If the question is outside the movie dataset, respond politely with:
// "Sorry, the information you requested is not available in the movie dataset."
// Do NOT make up information.`
//       },
...
//     // Step 6: If the RAG answer indicates "not available", also clear the movies array before returning
//     const finalMovies = (typeof ragAnswer === "string" && ragAnswer.toLowerCase().includes("not available in the movie dataset"))
//       ? []
//       : movies;

//     // Step 6: Return results
//     return NextResponse.json({
//       rag: { answer: ragAnswer, movies: finalMovies },
...
// app/api/search/route.js
import { getNeo4jDriver } from "@/lib/neo4j";
import { generateEmbedding, generateResponse, classifyMovieDomain } from "@/lib/openai";
import { NextResponse } from "next/server";
import OpenAI from "openai";
import { config } from "@/lib/config";
...
export async function POST(req) {
  try {
    ...
    // Step 0: (NEW) Fast domain check BEFORE any embeddings or DB calls
    const { isMovieDomain } = await classifyMovieDomain(query);
    if (!isMovieDomain) {
      const guardMsg = `Sorry, the information you requested is not available in the movie dataset.`;
      return NextResponse.json({
        rag: { answer: guardMsg, movies: [] },
        baseline: { answer: guardMsg },
        graph: { nodes: [], links: [] },
        metrics: {
          queryTime: 0,
          totalMovies: 0,
          queryType: "out_of_domain",
          vectorCount: 0,
          graphCount: 0,
          traversalDepth: 0,
          weights: { vector: 0, graph: 0 }
        }
      });
    }
...
Do NOT make up information.`
    ...
    // If model decided it's not available, kill graph
    if (typeof ragAnswer === "string" && ragAnswer.toLowerCase().includes("not available in the movie dataset")) {
      graphData = { nodes: [], links: [] };
    }
